...
func batchInsertWorker(client pb.LogServiceClient, batchChan <-chan []*pb.LogEntry, wg *sync.WaitGroup, workerID int) {
	...
	for batch := range batchChan {
		// 添加延迟以避免过载服务器
		time.Sleep(time.Duration(BATCH_INTERVAL) * time.Millisecond)
		...
		batchCount++
		if err != nil {
			log.Printf("Worker %d - Batch %d failed with error: %v", workerID, batchCount, err)
			continue
		}
		...
		successCount++
		if batchCount%50 == 0 {
			log.Printf("Worker %d - Batch %d completed, total success: %d", workerID, batchCount, successCount)
		}
	}

	log.Printf("Worker %d completed, processed %d batches, success: %d", workerID, batchCount, successCount)
}
...
func main() {
	log.Printf("继续插入剩余测试数据...")
	log.Printf("配置: 总记录数=%d, 批次大小=%d, 并发数=%d, 间隔=%dms", TOTAL_RECORDS, BATCH_SIZE, CONCURRENT_WORKERS, BATCH_INTERVAL)
